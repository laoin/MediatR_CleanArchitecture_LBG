    ...
    public static class SafeExecutionExtensions
    {
        public static async Task<Microsoft.AspNetCore.Http.IResult> Execute(Func<Task<Microsoft.AspNetCore.Http.IResult>> action, ILogger logger)
        {
            ...
            catch (Exception ex)
            {
                ...
                var responseError = new InHub.Connectors.Contracts.Common.Results.ValueResult<IEnumerable<UnsubmittedDataBriefDto>>
                {
                    Status = InHub.Connectors.Contracts.Common.Results.ResultStatus.Error,
                    Errors = new List<string> { ex.Message }
                };
